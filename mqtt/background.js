...
if (!localStorage.isInitialized) {
  ...
  localStorage.broker = "mosca-srv.herokuapp.com"; // broker websocket address
...
function onConnect() {
  console.log(
    "Connected to broker, subscribing for subtopic: " + localStorage.subtopic
  );
...
function onConnectionLost(responseObject) {
  if (responseObject.errorCode !== 0) {
    ...
    if (localStorage.reconnectTimeout !== 0) {
      window.setTimeout(
        connect,
        parseInt(localStorage.reconnectTimeout, 10) * 1000
      ); //wait X seconds before trying to connect again.
    }
  }
}
...
chrome.browserAction.onClicked.addListener(function() {
  console.log(
    "MQTT extension icon clicked, disconnect and reconnect to broker in progress"
  );
  try {
    client.disconnect();
    chrome.browserAction.setIcon({ path: "icon_noconnection.png" });
