...
export default class GitLabFileDownloader {
  ...
  public static download(
    ...
    if (file.absolutefilename === '') {
      ...
      chrome.downloads.download({
        url: `${file.origin}/api/v4/projects/${encodeURIComponent(
          file.reponame
  ...
  private static async download_file(file, showSaveAs) {
    // find hash of file
    let url = `${file.origin}/api/v4/projects/${encodeURIComponent(
      file.reponame
    )}/repository/tree?path=${encodeURIComponent(file.path)}&per_page=100`;
    if (file.branch) {
  ...
  private static async download_zip(file, showSaveAs) {
    ...
    let blobs = data
      ...
      .map(object => {
        return {
          content: fetch(
            `${file.origin}/api/v4/projects/${encodeURIComponent(
              file.reponame
            )}/repository/blobs/${object.id}/raw`,
            { credentials: 'include' }
          ).then(res => res.blob()),
          ...object
        };
