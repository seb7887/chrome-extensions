...
export default class GitHubFileDownloader {
  ...
  public static download(
    ...
    let notificationId: string = NotificationHelper.create(
      'Progress download',
      '',
      2,
      true
    ...
    githubURL.getDownloadUrl().then(
      function(url: string) {
        let filename = null;

        if (
          githubURL.fileType !== FileType.ZIPBALL &&
          githubURL.fileType !== FileType.TREE
        ) {
          filename = githubURL.filePath
            .split('/')
            .pop()
            .replace(/^[.]+/g, '');
